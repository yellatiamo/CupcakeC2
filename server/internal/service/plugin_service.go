...
// matching the common SA-BOF packing form "Zs":
//
//	[big-endian u32 byte-length][UTF-16LE path, wide-NUL terminated][big-endian u16 subdirs]
//
// BOFs such as dir.x64 cast BeaconDataExtract to wchar_t* and immediately call
// MSVCRT$wcsncat(buf, path, …). A NULL path (empty args) AVs inside msvcrt.dll
// (APPCRASH c0000005). Empty / whitespace-only args therefore default to ".".
//
// Optional trailing " /s" or " -s" (case-insensitive) sets the subdirs short to 1
// and is stripped from the path (CS dir BOF recursive flag).
func packBofArgsWide(args string) []byte {
	path := strings.TrimSpace(args)
	var subdirs uint16
	// Strip recursive flags commonly typed by operators.
	for {
		lower := strings.ToLower(path)
		switch {
		case strings.HasSuffix(lower, " /s"):
			path = strings.TrimSpace(path[:len(path)-3])
			subdirs = 1
			continue
		case strings.HasSuffix(lower, " -s"):
			path = strings.TrimSpace(path[:len(path)-3])
			subdirs = 1
			continue
		case strings.HasSuffix(lower, "\t/s"), strings.HasSuffix(lower, "\t-s"):
			path = strings.TrimSpace(path[:len(path)-3])
			subdirs = 1
			continue
		}
	...
	if path == "" {
		// dir.x64: go() does wcsncat(realPath, BeaconDataExtract(...), …) with no
		// NULL check. Default to current directory instead of packing nothing.
		path = "."
	}
	units := utf16.Encode([]rune(path))
	units = append(units, 0) // wide NUL terminator
	pathBytes := len(units) * 2
	buf := make([]byte, 4+pathBytes+2)
	binary.BigEndian.PutUint32(buf[:4], uint32(pathBytes))
	for i, u := range units {
		binary.LittleEndian.PutUint16(buf[4+i*2:], u)
	}
	binary.BigEndian.PutUint16(buf[4+pathBytes:], subdirs)
	return buf
}
