...
pub fn map_pe_opts(pe: &[u8], call_dll_main: bool) -> Result<MappedModule, String> {
    // CFG relax only when Manual-Map actually runs — not at connect/startup.
    #[cfg(windows)]
    crate::stealth::ensure_cfg_relaxed();

    if pe.len() < 0x40 {
        return Err("map: buffer too small".into());
    }
    if pe[0] != b'M' || pe[1] != b'Z' {
        return Err("map: not MZ".into());
    }

    let dos_e_lfanew = u32::from_le_bytes([pe[0x3C], pe[0x3D], pe[0x3E], pe[0x3F]]) as usize;
    if dos_e_lfanew + 0x18 > pe.len() {
        return Err("map: bad e_lfanew".into());
    }
    let nt = dos_e_lfanew;
    let sig = u32::from_le_bytes([pe[nt], pe[nt + 1], pe[nt + 2], pe[nt + 3]]);
    if sig != IMAGE_NT_SIGNATURE {
        return Err("map: bad PE signature".into());
    }

    let machine = u16::from_le_bytes([pe[nt + 4], pe[nt + 5]]);
    #[cfg(target_arch = "x86_64")]
    if machine != IMAGE_FILE_MACHINE_AMD64 {
        return Err(format!("map: machine 0x{machine:x} != amd64"));
    }
    #[cfg(target_arch = "x86")]
    if machine != IMAGE_FILE_MACHINE_I386 {
        return Err(format!("map: machine 0x{machine:x} != i386"));
    }
    ...
    if opt + opt_hdr_size > pe.len() {
        return Err("map: truncated optional header".into());
    }
    ...
    let (
        ...
        return Err(format!("map: unknown optional magic 0x{magic:x}"));
    };
    ...
    // runs x0..x3 — any thread_local access there would AV at the
    // gs:[0x58] accessor, so the MODULE BUILD CONTRACT forbids touching
    // thread_local in x0..x3 (executor crates and std TLS-touching paths are
    // kept out of module builds). With that contract the image can run with
    // a NEUTERED TLS directory: we zero the directory and write a sentinel
    // index before DllMain (see `neuter_tls` below) so a stray TLS access
    // takes a clean AV instead of dereferencing stale pointers, and the
    // loader never runs TLS callbacks (image is not loader-registered).
    // Every Rust cdylib links std statics (THREAD_INFO & co.), so a TLS
    // directory is unavoidable in practice — refusing it would reject ALL
    // modules. x86 stays hard-refused: its TLS model cannot be neutralized.
    #[cfg(target_arch = "x86")]
    if tls_rva != 0 {
        return Err(
            "map: TLS directory present — x86 Manual-Map unsupported (use OS loader fallback)"
                .into(),
        );
    }

    if size_of_image == 0 || size_of_image > 256 * 1024 * 1024 {
        return Err(format!("map: absurd SizeOfImage {size_of_image}"));
    }
    if size_of_headers > pe.len() || size_of_headers > size_of_image {
        return Err("map: bad SizeOfHeaders".into());
    }

    let section_table = opt + opt_hdr_size;
    if section_table + num_sections * 40 > pe.len() {
        return Err("map: truncated section table".into());
    }

    let base_ptr = nt_alloc_rw(size_of_image, true).map_err(|e| format!("map alloc: {e}"))?;
    let base = base_ptr as usize;
    crate::db_print!(
        "[mapper] alloc base=0x{:X} soi=0x{:X} hdrs=0x{:X} entry_rva=0x{:X} tls_rva=0x{:X} reloc_rva=0x{:X}",
    ...
    for i in 0..num_sections {
        ...
        if raw_ptr + raw_size > pe.len() {
            ...
            return Err(format!("map: section {i} raw out of bounds"));
        }
        if va >= size_of_image {
            continue;
        }
    ...
    crate::db_print!("[mapper] sections copied");
    ...
    crate::db_print!("[mapper] relocs done delta=0x{:X}", delta as usize);
    ...
    crate::db_print!("[mapper] imports resolved");

    // Static TLS: neuter on BOTH paths (export-probe and real load). The
    // module build contract (no thread_local access in x0..x3) makes the
    // sentinel index + zeroed directory safe — a stray local-exec TLS read
    // takes a clean AV at the access site, and the loader never runs TLS
    // callbacks for a manually-mapped image.
    if tls_rva != 0 {
    ...
    crate::db_print!("[mapper] tls handled");
    ...
    {
        warn!("[mapper] protect_sections: {e}");
        // non-fatal: leave RW
    }
    crate::db_print!("[mapper] protections set");
    ...
    if call_dll_main && entry_rva != 0 && entry_rva < size_of_image {
        ...
        crate::db_print!(
            "[mapper] calling DllMain entry=0x{:X}",
            base + entry_rva
        );
        let ok = unsafe { entry(base as *mut _, DLL_PROCESS_ATTACH, std::ptr::null_mut()) };
        crate::db_print!("[mapper] DllMain returned {}", ok);
        if ok == 0 {
            unsafe {
                wipe_and_free(base_ptr, size_of_image);
            }
            return Err("map: DllMain(DLL_PROCESS_ATTACH) failed".into());
        }
    ...
    if mod_invoke.is_none() {
        ...
        return Err("map: required export missing".into());
    }
    ...
    // own protections.)
    // DIAG: APP_KEEP_HEADERS=1 skips the wipe so the crash capture can dump
    // the true loaded section table.
    if std::env::var("APP_KEEP_HEADERS").is_err() {
        unsafe {
    ...
    debug!(
        "[mapper] mapped image base=0x{base:x} size=0x{size_of_image:x} dll_main={dll_main_called}"
    );
...
unsafe fn neuter_tls(
    ...
    if tls_rva + 40 > size_of_image {
        return Err("map: TLS directory OOB".into());
    }
    ...
    if index_ptr < base || index_ptr + 4 > base + size_of_image {
        return Err("map: TLS AddressOfIndex outside image".into());
    }
...
unsafe fn neuter_tls(
    ...
    Err("map: TLS neutering unsupported on this target".into())
}
...
unsafe fn apply_relocs(
    ...
    if reloc_rva + reloc_size > size_of_image {
        return Err("map: reloc dir OOB".into());
    }
    let mut off = 0usize;
...
unsafe fn resolve_imports(
    ...
    if k32 == 0 {
        return Err("map: kernel32 missing".into());
    }
    ...
    loop {
        ...
        let dll_name = read_cstr(base + name_rva, base + size_of_image)
            .ok_or_else(|| "map: bad import DLL name".to_string())?;
        let hmod = {
            ...
            if h != 0 {
                ...
                let c = std::ffi::CString::new(dll_name.as_str())
                    .map_err(|_| "map: dll name cstring".to_string())?;
                let p = load_lib(c.as_ptr());
                if p.is_null() {
                    return Err(format!("map: LoadLibraryA failed for {dll_name}"));
                }
                p
            }
        };
        ...
        // IMAGE_ORDINAL_FLAG: bit 63 on x64, bit 31 on x86 — always widen to u64 for compare
        #[cfg(target_arch = "x86_64")]
        let ord_flag: u64 = 0x8000_0000_0000_0000;
        #[cfg(target_arch = "x86")]
        let ord_flag: u64 = 0x8000_0000;

        loop {
            #[cfg(target_arch = "x86_64")]
            let raw = *(oft as *const u64);
            #[cfg(target_arch = "x86")]
            let raw = u64::from(*(oft as *const u32));
            ...
            let proc = if (raw & ord_flag) != 0 {
                ...
                if hint_name + 2 >= base + size_of_image {
                    return Err("map: import name OOB".into());
                }
                let name_ptr = (hint_name + 2) as *const i8;
                get_proc(hmod, name_ptr)
            };
            if proc.is_null() {
                return Err("map: GetProcAddress failed for import".into());
            }
            #[cfg(target_arch = "x86_64")]
            {
...
pub fn mem_map_enabled() -> bool {
    // Env name built at runtime (avoid contiguous PE IoC for map toggles).
    let env_name = {
        let mut n = String::from("SVC");
        n.push('_');
        n.push_str("IMG");
        n.push('_');
        n.push_str("MAP");
...
/// Always true (LoadLibrary fallback was removed entirely).
...
mod tests {
    ...
    fn map_real_product_exports_without_dllmain() {
        let Some(path) = find_product_l2_pe() else {
            eprintln!("skip: no product L2 PE (set TEST_MOD_PE or build inject)");
            return;
        };
        ...
        let m = map_pe_opts(&pe, false).unwrap_or_else(|e| {
            panic!("map success path required for product PE, got Err: {e}")
        });
        ...
        unmap_pe(&m);
        eprintln!("OK map export probe success path on {}", path.display());
    }
    ...
    fn map_real_product_full_dllmain_clean_err_or_ok() {
        ...
        match map_pe(&pe) {
            Ok(m) => {
                ...
                eprintln!(
                    "OK map full DllMain map base=0x{:x} size={}",
                    m.base, m.size
                );
                unmap_pe(&m);
            }
            Err(e) => {
                eprintln!("map full DllMain clean Err (LoadLibrary fallback): {e}");
                assert!(
                    e.contains("TLS")
                        || e.contains("DllMain")
                        || e.contains("map")
